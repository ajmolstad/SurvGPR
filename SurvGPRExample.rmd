...
---
In this document, we show how to use the $\texttt{SurvGPR}$ R package. **If you encounter any errors or strange behavior, please report the issue at http://www.github.com/ajmolstad/SurvGPR.** First, we download the package from GitHub. 
...
pred_results <- SurvGPR_Predict(results = results, barT = results$Tout , Z.full = Z, K.full = K.test,
                train.inds = train.inds, test.inds = test.inds, times = seq(0, max(time), length=50))
str(pred_results)
```
...
```{r survPlot, cache=TRUE, tidy=TRUE}
plot(x = seq(0, max(time), length=50), y = pred_results$survFunc[1,], 
