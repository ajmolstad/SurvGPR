...
\arguments{
 \item{time}{An \eqn{n}-variate or containing the failure/censoring times (on the original scale -- NOT log-transformed).}
  ...
  \item{initializer}{Only used when \code{kern.type = "multi-K"}: either \eqn{0,1} indicating the type of initialization used for the Monte-Carlo EM. The default is 0,  sets all variance componets equal and gets an initial estimate of \eqn{\hat{\beta}} using the updating equation from Algorithm 2. 
                     The code 1 which fits a variance components model to the IPW-mean-imputed dataset.}
